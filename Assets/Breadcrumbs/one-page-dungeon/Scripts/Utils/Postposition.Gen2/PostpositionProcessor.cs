...
namespace Breadcrumbs.Utils.Gen2 {
    ...
    public static class PostpositionProcessor {
        ...
        // 언어별 조사 처리 전략 캐시
        private static readonly Dictionary<LanguageType, IPostpositionStrategy> StrategyCache = new();

        // 조사 타입별 기본 조사 문자열 매핑
        private static readonly Dictionary<PostpositionType, string> PostpositionStringMap = new() {
        ...
        public static string ApplyPostposition(string word, string postposition) {
            ...
            try {
                // 단어의 마지막 문자 언어 유형 확인
                var languageType = CharacterClassifier.GetLanguageType(word);

                // 해당 언어에 맞는 전략 가져오기
                var strategy = GetStrategy(languageType);
        ...
        public static string ProcessMessage(string message, Dictionary<string, string> keywordMap) {
            ...
            try {
                ...
                return PatternRegex.Replace(message, match => {
                    ...
                    var postposition = match.Groups[2].Value;

                    // 키워드 매핑이 존재하면 조사 적용
                    if (keywordMap.TryGetValue(keyword, out var replacement)) {
        ...
        private static IPostpositionStrategy GetStrategy(LanguageType languageType) {
            // 캐시에서 전략 확인
            if (!StrategyCache.TryGetValue(languageType, out var strategy)) {
                // 캐시에 없으면 팩토리에서 생성
