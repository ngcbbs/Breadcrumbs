...
namespace Breadcrumbs.Utils.Gen2 {
    ...
    public static class EnglishToKoreanConverter {
        ...
        private static readonly Dictionary<string, bool> ExceptionDictionary =
        ...
        private static readonly Regex HasBatchimPattern =
            new Regex(@"(?i)(ion|[bcdfghjklmnpqrstvwxz]|[^aeiouy]e|ome)$", RegexOptions.Compiled);

        // 받침이 없는 발음으로 끝나는 단어 패턴
        private static readonly Regex NoBatchimPattern =
            new Regex(@"(?i)([aeiou]|y|ed|ue|ee|ie|oe|ye|th)$", RegexOptions.Compiled);
        ...
        public static bool HasBatchim(string word) {
            ...
            if (word.EndsWith("ed", StringComparison.OrdinalIgnoreCase) &&
                word.Length > 2 && IsConsonant(word[^3]))
                return false;
