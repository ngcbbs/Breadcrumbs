...
namespace Breadcrumbs.Utils.Gen2.Example
{
    ...
    public class PostpositionProcessor_SimpleExample : MonoBehaviour
    {
        void Start()
        {
            ...
            // 예제 1: 기본 조사 처리
            string result1 = PostpositionProcessor.ApplyPostposition("사과", "은/는");
            string result2 = PostpositionProcessor.ApplyPostposition("책", "은/는");
            string result3 = PostpositionProcessor.ApplyPostposition("apple", "은/는");
            
            Debug.Log($"1. 기본 조사 처리: {result1}, {result2}, {result3}");
            // 출력: 1. 기본 조사 처리: 사과는, 책은, apple은
            
            // 예제 2: 조사 타입 사용
            string result4 = PostpositionProcessor.ApplyPostposition("사과", PostpositionType.Subject);
            string result5 = PostpositionProcessor.ApplyPostposition("책", PostpositionType.Object);
            
            Debug.Log($"2. 조사 타입 사용: {result4}, {result5}");
            // 출력: 2. 조사 타입 사용: 사과는, 책을
            
            // 예제 3: 영어 단어 처리
            string result6 = PostpositionProcessor.ApplyPostposition("apple", "이/가");
            string result7 = PostpositionProcessor.ApplyPostposition("computer", "이/가");
            string result8 = PostpositionProcessor.ApplyPostposition("google", "을/를");
            
            Debug.Log($"3. 영어 단어 처리: {result6}, {result7}, {result8}");
            // 출력: 3. 영어 단어 처리: apple이, computer가, google을
            
            // 예제 4: 숫자 처리
            string result9 = PostpositionProcessor.ApplyPostposition("1", "이/가");
            string result10 = PostpositionProcessor.ApplyPostposition("2", "이/가");
            string result11 = PostpositionProcessor.ApplyPostposition("10", "을/를");
            
            Debug.Log($"4. 숫자 처리: {result9}, {result10}, {result11}");
            // 출력: 4. 숫자 처리: 1이, 2가, 10을
            
            // 예제 5: 메시지 패턴 처리
            string message = "안녕하세요, {이름:은/는} {역할:이/가} {업무:을/를} 담당하고 있습니다.";
            ...
            string message2 = "{Name:이/가} {Job:으로/로} {Department:에서/에서} 근무합니다.";
