...
namespace Breadcrumbs.CharacterSystem {
    public class PlayerCharacter : MonoBehaviour {
        ...
        private void Awake() {
            // 서비스 로케이터에 등록
            ServiceLocator.RegisterService<PlayerCharacter>(this);
        }
        ...
        public void InitializeCharacter() {
            ...
            // 시작 스킬 추가
            ISkillManager skillManager = ServiceLocator.GetService<ISkillManager>();
        ...
        public void GainExperience(int amount) {
            ...
            if (leveledUp) {
                ...
                // 이벤트 발생
                EventManager.Trigger("Character.LevelUp", new LevelUpEventData(this, stats.Level, statPoints, skillPoints));
            }
        }
        ...
        private void CheckNewSkillsAtLevel() {
            // 스킬 데이터베이스 접근
            var skillDb = ServiceLocator.GetService<SkillDatabase>();
            if (skillDb != null) {
                // 현재 레벨에서 배울 수 있는 스킬 찾기
                var newSkills = SkillDatabase.GetSkillsByLevel(classType, stats.Level);
                ISkillManager skillManager = ServiceLocator.GetService<ISkillManager>();
        ...
        public bool UseSkill(string skillId, Transform target = null) {
            ISkillManager skillManager = ServiceLocator.GetService<ISkillManager>();
            return skillManager?.UseSkill(skillId, target) ?? false;
        }
        ...
        public bool UseStatPoint(StatType statType) {
            ...
            // 이벤트 발생
            EventManager.Trigger("Character.StatChanged", new StatChangedEventData(this, statType, 1, statPoints));
        ...
        public bool UseSkillPoint(string skillId) {
            ...
            ISkillManager skillManager = ServiceLocator.GetService<ISkillManager>();
            if (skillManager != null && skillManager.LevelUpSkill(skillId)) {
                skillPoints--;

                // 이벤트 발생
                EventManager.Trigger("Character.SkillPointUsed", new SkillPointUsedEventData(this, skillId, skillPoints));
        ...
        public bool EquipItem(EquipmentItem item) {
            ...
            // 아이템을 인벤토리에 추가한 후 장착 - 구현에 따라 다름
            // 여기서는 단순화를 위해 인벤토리 시스템에 위임

            // 이벤트 발생
            EventManager.Trigger("Character.ItemEquipped", new ItemEquippedEventData(this, item));
        ...
        public void DisplayCharacterInfo() {
            ...
            Debug.Log(
                $"Attack: Physical {stats.GetStat(StatType.PhysicalAttack)}, Magical {stats.GetStat(StatType.MagicAttack)}");
            Debug.Log(
                $"Defense: Physical {stats.GetStat(StatType.PhysicalDefense)}, Magical {stats.GetStat(StatType.MagicDefense)}");
            Debug.Log(
                $"Crit: {stats.GetStat(StatType.CriticalChance) * 100}% chance, {stats.GetStat(StatType.CriticalDamage)}x damage");
        ...
        // 캐릭터 데이터 로드
        public void LoadCharacterData(CharacterSaveData saveData) {
            if (saveData == null)
                return;
            ...
            if (inventory != null && itemDatabase != null) {
                ...
                foreach (var itemPair in saveData.equippedItems) {
                    ...
                    if (item != null) {
                        // todo: fixme
                        /*
                        // 인벤토리에 추가 후 장착 로직 (실제 구현에 맞게 수정 필요)
            ...
            // 로드 완료 이벤트 발생
            EventManager.Trigger("Character.DataLoaded", this);
        }
    }
    ...
    public class StatChangedEventData {
        ...
        public StatChangedEventData(PlayerCharacter character, StatType statType, float value, int remainingPoints) {
            Character = character;
            StatType = statType;
            Value = value;
    ...
    public class SkillPointUsedEventData {
        public PlayerCharacter Character { get; private set; }
        public string SkillId { get; private set; }
        public int RemainingPoints { get; private set; }

        public SkillPointUsedEventData(PlayerCharacter character, string skillId, int remainingPoints) {
            Character = character;
            SkillId = skillId;
            RemainingPoints = remainingPoints;
        }
    ...
    public class ItemEquippedEventData {
        ...
        public ItemEquippedEventData(PlayerCharacter character, EquipmentItem item) {
            Character = character;
            Item = item;
        }
    }
