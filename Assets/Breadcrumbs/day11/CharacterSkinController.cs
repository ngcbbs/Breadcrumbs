...
namespace Breadcrumbs.day11 {
    public class CharacterSkinController : MonoBehaviour {
        ...
        public bool changeMaterialAtSpawn = false;
        ...
        void Start() {
            ...
            if (changeMaterialAtSpawn) {
                if (0 <= materialIndex && materialIndex < characterMaterials.Length)
                    ChangeMaterialSettings(materialIndex);
                else 
                    Debug.LogWarning($"{materialIndex} is not a valid material");
            }
        }
        ...
        void ChangeMaterialSettings(int index) {
            for (int i = 0; i < characterMaterials.Length; i++) {
                if (characterMaterials[i].transform.CompareTag("PlayerEyes"))
                    ...
                    // URP/Lit base texture change. :P
                    characterMaterials[i].material.SetTexture("_BaseMap", albedoList[index]);
                    //Debug.Log($"SetTexture: {albedoList[index].name} -> {characterMaterials[i].name}");
                }
            }
        }
